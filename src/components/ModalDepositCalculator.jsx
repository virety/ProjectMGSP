...
const ModalDepositCalculator = ({ show, onClose }) => {
  ...
  const handleAmountChange = (value) => {
    const newAmount = Math.min(Math.max(value, 1000), 10000000);
    setAmount(newAmount);
  };

  const handleTermChange = (value) => {
    const newTerm = Math.min(Math.max(value, 1), 36);
    setTerm(newTerm);
  };
      ...
      <div className="deposit-calculator">
        ...
        <div className="calculator-form">
          ...
          <div className="rate-display">
            Процентная ставка: <span className="rate-value">{DEPOSIT_RATE * 100}% годовых</span>
          </div>
