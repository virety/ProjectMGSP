...
struct SendMoneyView: View {
    ...
    private func sendMoney() {
        ...
        case .cardNumber:
            guard isValidCardNumber(cleanCard) else {
        ...
        guard let card = cards.first else {
            showError(message: "Карта не найдена")
            return
        }

        if card.balance < amount {
            showError(message: "Недостаточно средств на карте")
            return
        }
        ...
        newTransaction.title = makeTitle()
        ...
        do {
            ...
            showError(message: "Не удалось сохранить транзакцию: \(error.localizedDescription)")
        }
    }
    ...
    private func makeTitle() -> String {
        ...
        switch selectedMethod {
        case .recipientName:
            base = "Перевод: \(recipientName)"
        case .phoneNumber:
            base = "Перевод по телефону: \(phoneNumber)"
        case .cardNumber:
            base = "Перевод по карте: \(cardNumber)"
        }
    ...
    private func isValidCardNumber(_ card: String) -> Bool {
