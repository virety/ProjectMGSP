...
struct BankHomeView: View {
    ...
    @FetchRequest(
        entity: Card.entity(),
        sortDescriptors: [NSSortDescriptor(keyPath: \Card.cardIssueDate, ascending: false)],
        predicate: NSPredicate(format: "owner.isAuthenticated == true")
    ) private var userCards: FetchedResults<Card>
    ...
    private func createBankCard(from card: Card) -> BankCard {
        let formatter = NumberFormatter()
        formatter.numberStyle = .decimal
        formatter.groupingSeparator = " "
        let balanceString = formatter.string(from: NSNumber(value: card.balance)) ?? "0"
        
        return BankCard(
            balance: "₽\(balanceString)",
            info: card.cardName ?? "",
            number: card.cardNumber ?? "**** **** **** ****",
            expiry: formatExpiryDate(card.cardExpiry),
            gradientStart: Color(red: 0.24, green: 0.18, blue: 0.91),
            gradientEnd: Color(red: 0.70, green: 0.28, blue: 1.00),
            image: nil
        )
                            ...
                            HStack(spacing: 16) {
                                ...
                                if let user = users.first {
                                    ...
                                    ForEach(userCards) { card in
                                        NavigationLink(destination: CardDetailView(card: createBankCard(from: card))) {
                                            BankCardView(card: createBankCard(from: card))
                                        }
                                        .buttonStyle(PlainButtonStyle())
                                    }
                                }
